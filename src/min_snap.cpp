...
namespace MIN_SNAP
{
    ...
    min_snap_traj::min_snap_traj(const std::vector<Eigen::Vector3d> &waypt_list, int m_dimension, bool close_form, bool verbose)
    ...
    double min_snap_traj::computeDistance(const std::vector<Eigen::Vector3d> &waypt_list, std::vector<double> &segment_length)
    {
        double total_dist = 0;
    ...
    std::vector<double> min_snap_traj::computeT(const std::vector<Eigen::Vector3d> &waypt_list, const Eigen::Vector3d &vel_max)
    {
    ...
    std::vector<double> min_snap_traj::computeT(const std::vector<Eigen::Vector3d> &waypt_list, double total_time)
    {
        // double vel_norm = vel_max.norm();

        int segment_number = waypt_list.size() - 1; // same as number of entries in time std::vector
    ...
    void min_snap_traj::computeHessian(Eigen::MatrixXd &hessian, int hessian_dim, int n_poly_order, int r_derivate_order, int k_segment, const std::vector<double> &time_vector)
    {
    ...
    void min_snap_traj::computeEquality(Eigen::MatrixXd &Aeq, int Aeq_row, int Aeq_col, Eigen::VectorXd &beq, int beq_dim, int n_poly_order, int k_segment, const std::vector<double> &waypoint_list, const std::vector<double> &time_vector)
    {
    ...
    void min_snap_traj::computeAeq(Eigen::MatrixXd &Aeq, int n_poly_order, int k_segment, const std::vector<double> &time_vector)
    {
        // (1) waypoints constraints (k+1)
        // Aeq(0,0) = 1;
        // for (int i = 1; i < k_segment; i++)
    ...
    void min_snap_traj::computeBeq(Eigen::VectorXd &beq, int k_segment, const std::vector<double> &waypoint_list)
    {
        int row_count = 0;
    ...
    void min_snap_traj::evaluatePoly(Eigen::VectorXd &row_vector, int derivative_order, double time, int polynomial_order)
    {
    ...
    Eigen::VectorXd min_snap_traj::optimize(const Eigen::MatrixXd &hessian_Q, const Eigen::MatrixXd &equality_A, Eigen::VectorXd &equality_b)
    ...
    void min_snap_traj::computeDerivateMappingMatrix(Eigen::MatrixXd &mapping_A, int k_segment, const std::vector<double> &time_vector)
    {
    ...
    void min_snap_traj::computeCoeffConstrainedQP(Eigen::VectorXd &coeff, const Eigen::MatrixXd &hessian_Q, int n_poly_order, int k_segment, const std::vector<double> &waypt_single_axis, const std::vector<double> &time_vector)
    {
    ...
    void min_snap_traj::computeCoeffCloseFormQPSingleAxis(Eigen::VectorXd &coeff, const Eigen::MatrixXd &hessian_Q, int n_poly_order, int k_segment, const std::vector<double> &waypt_single_axis, const std::vector<double> &time_vector)
    {
    ...
    void min_snap_traj::computeCoeffCloseFormQPThreeAxis(std::vector<Eigen::MatrixXd> &coeff_matrix, const Eigen::MatrixXd &hessian_Q, int n_poly_order, int k_segment, const std::vector<Eigen::Vector3d> &waypt_list, const std::vector<double> &time_vector)
    {
        // closed form solutions

        // transform waypt_list into matrix
        Eigen::MatrixXd waypt(m_dimension, k_segment + 1);
        ...
        Eigen::MatrixXd decision_f = Eigen::MatrixXd::Zero(fixed_number, m_dimension);
        ...
        Eigen::MatrixXd decision_p_primal = Eigen::MatrixXd::Zero(free_number, m_dimension);
        ...
        decision_p_primal = -R_pp.inverse() * R_fp.transpose() * decision_f;
        Eigen::MatrixXd decision_primal = Eigen::MatrixXd::Zero(fixed_number + free_number, m_dimension);
        decision_primal << decision_f, decision_p_primal;

        Eigen::MatrixXd derivative_vector = Eigen::MatrixXd::Zero(k_segment * (constraint_order + 1) * 2, m_dimension);
    ...
    void min_snap_traj::coeffMatReshape(const Eigen::MatrixXd &coeff_all, std::vector<Eigen::MatrixXd> &coeff)
    {
    ...
    poly_traj min_snap_traj::computeTrajectory(const std::vector<Eigen::Vector3d> &waypt_list)
    {
        ...
        std::vector<double> time_vector = this->computeT(waypt_list, time);
        ...
        else
        {
            ...
            Eigen::MatrixXd coeff_all = Eigen::MatrixXd::Zero(k_segment * (n_order + 1), m_dimension);
